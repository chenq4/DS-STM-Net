...
class medt_net(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=1, zero_init_residual=True,
                 groups=2, width_per_group=16, replace_stride_with_dilation=None,
                 norm_layer=None, s=0.125, img_size = 128,imgchan = 3):
    ...
    def _forward_impl(self, x):
        ...
        for i in range(0,patch_number):
            for j in range(0,patch_number):

                x_p = xin[:,:,patch_size*i:patch_size*(i+1),patch_size*j:patch_size*(j+1)]
                # begin patch wise
                ...
                x_loc[:,:,patch_size*i:patch_size*(i+1),patch_size*j:patch_size*(j+1)] = x_p
